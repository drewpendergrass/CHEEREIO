...
animation_fps="$(jq -r ".animation_fps" ../ens_config.json)"
...
for i in "${controlvec[@]}"
do
	python animator.py "${concpp}" "SpeciesConc_${i}" "all" "${pp_dir}/SpeciesConc_${i}" "${animation_fps}"
done

for i in "${emisvec[@]}"
do
	python animator.py "${pp_dir}/${i}_SCALEFACTOR.nc" "Scalar" "all" "${pp_dir}/SCALEFACTOR_${i}" "${animation_fps}"
done

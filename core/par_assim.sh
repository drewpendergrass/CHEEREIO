...
python par_letkf.py ${end_timestamp} ${1} ${2} >> ${MY_PATH}/${RUN_NAME}/ensemble_runs/logs/letkf_${1}_${2}.out
py_exit_status=$?
source deactivate #Exit Conda environment

#A cleanup script will check if everyone is done
printf "done" > ${MY_PATH}/${RUN_NAME}/scratch/complete_subprocess_${1}_${2}
